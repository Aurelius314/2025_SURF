...
# def query_similar_features_optimized(
#     query_vector: np.ndarray | torch.Tensor,
#     table_names: list[str],
#     record_column_name: str,
#     vector_column: str,
...
#                 {record_column_name}, 
#                 1 - ({vector_column} <=> '[{query_vector_str}]') AS similarity,
#                 {vector_column}::text AS feature_str
#             FROM {table}
#         """
#         subqueries.append(sub_sql)
...
#     # 使用DISTINCT ON去重，保留相似度最高的记录
#     final_sql = f"""
#         SELECT table_name, {record_column_name}, similarity FROM (
#             SELECT DISTINCT ON (feature_str) 
#                 table_name, 
#                 {record_column_name}, 
#                 similarity,
#                 feature_str
...
#                 {union_sql}
#             ) AS all_results
#             ORDER BY feature_str, similarity DESC
